...
namespace LibraryManageSystem
{
    public partial class ReaderMain : Form
    {
        ...
        private void tabControl1_Click(object sender, EventArgs e)//更改个人信息时的事件
        {
            ...
            string webpic = "http://cdn2.jie12366.xyz/users/" + lblNum.Text + ".jpg";
            pictureUser.ImageLocation = webpic;
        }
        private void tabControl4_Click(object sender, EventArgs e)//还书界面自动触发事件
        {
            cmbReturn.Items.Clear();
            DataSet dr = DBOperate.readDB("select Books.BookName 书名,Books.ISBN 书号,Books.BookStyle 类型,BorrowTime 借出时间,ReturnTime 应归还时间 from BRBooks INNER JOIN Books  on BRBooks.ISBN=Books.ISBN where BRBooks.UserNum='" + s + "'");
        ...
        private void btnSearchPic_Click(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string webpic = "http://cdn2.jie12366.xyz/books/" + ISBN + ".jpg";//拉取阿里云oss对应图片
